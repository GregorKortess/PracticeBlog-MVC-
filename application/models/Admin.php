...
class Admin extends Model
{
    ...
    public function loginValidate($post)
    {
        ...
        if($config['login'] != $post['login'] or $config['password'] !=$post['password']) {
            $this->error = 'Логин или пороль указан не верно';
            return false;
        }
        return true;
    }
    ...
    public function postValidate($post, $type)
    {
        ...
        return false;
        } elseif ($textLen <10 or $textLen > 7000) {
            $this->error = 'Текст должен содержать от 10 до 7000 символов';
            return false;
        }
    ...
    {
        // Надо сделать обработку изображений если на хостинге есть IMAGICK
